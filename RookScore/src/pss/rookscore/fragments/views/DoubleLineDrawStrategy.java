...
public class DoubleLineDrawStrategy implements DrawStrategy {
    ...
    public DoubleLineDrawStrategy(Context context, Paint p, List<String> playerNames, List<RoundSummary> roundSummaries, int totalWidth) {
        mContext = context;
        mPaint = p;
    ...
    public void drawRoundSummary(Context context, Canvas c, RoundSummary summary) {
        //we are ready to draw the first line
         ...
         ViewUtilities.summarizeFirstLineRoundSummary(roundSummaryText, mPlayerNames, summary.getRoundResult());
         ...
         ViewUtilities.summarizeSecondLineRoundResult(roundSummaryText, mPlayerNames, summary.getRoundResult());
         c.drawText(roundSummaryText.toString(), 0, -ViewUtilities.scaleText(context, 4), mPaint);
    ...
    public float computeRoundSummaryWidth(List<RoundSummary> roundSummaries) {
        ...
        for (RoundSummary roundSummary : roundSummaries) {
            ViewUtilities.summarizeFirstLineRoundSummary(roundSummaryText, mPlayerNames, roundSummary.getRoundResult());
           
            maxWidth = Math.max(maxWidth, mPaint.measureText(roundSummaryText.toString()));
            
            roundSummaryText.setLength(0);
            ViewUtilities.summarizeSecondLineRoundResult(roundSummaryText, mPlayerNames, roundSummary.getRoundResult());
            maxWidth = Math.max(maxWidth, mPaint.measureText(roundSummaryText.toString()));
            roundSummaryText.setLength(0);
